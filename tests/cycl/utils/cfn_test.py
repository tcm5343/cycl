...
from cycl.utils.cfn import parse_name_from_id, list_all_exports, get_imports


@pytest.fixture
def cfn_client_mock():
    cfn_client_mock = Mock(name='cfn_client_mock')
...
def test_list_all_exports_returns_an_export(expected_exports, mock_boto3, cfn_client_mock):
    cfn_client_mock.list_exports.return_value = {'Exports': expected_exports}

    actual_exports = list_all_exports()

    mock_boto3.client.assert_called_once_with('cloudformation')
    assert actual_exports == expected_exports


def test_list_all_exports_conditionally_creates_client(mock_boto3, cfn_client_mock):
    expected_exports = []
    cfn_client_mock.list_exports.return_value = {'Exports': expected_exports}

    actual_exports = list_all_exports(cfn_client=cfn_client_mock)

    mock_boto3.client.assert_not_called()
    assert actual_exports == expected_exports


def test_list_all_exports_uses_next_token(mock_boto3, cfn_client_mock):
...
def test_get_imports_returns_imports(expected_imports, mock_boto3, cfn_client_mock):
    export_name = 'some-export_name'
    cfn_client_mock.list_imports.return_value = {'Imports': expected_imports}

    actual_imports = get_imports(export_name=export_name)
...
def test_get_imports_conditionally_creates_client(mock_boto3, cfn_client_mock):
    export_name = 'some-export_name'
    expected_imports = []
    cfn_client_mock.list_imports.return_value = {'Imports': expected_imports}

    actual_imports = get_imports(export_name=export_name, cfn_client=cfn_client_mock)

    mock_boto3.client.assert_not_called()
    assert actual_imports == expected_imports


def test_get_imports_uses_next_token(mock_boto3, cfn_client_mock):
    export_name = 'some-export_name'
...
def test_get_imports_excepts_client_error(mock_boto3, cfn_client_mock):
    export_name = 'some-export_name'
    # todo: determine what this error message actually looks like
    cfn_client_mock.list_imports.side_effect = ClientError(
        {'Error': {'Code': 'SomeErrorCode', 'Message': 'is not imported by any stack'}}, 'some_operation'
    )
...
def test_get_imports_raises_client_error(mock_boto3, cfn_client_mock):
    export_name = 'some-export_name'
    cfn_client_mock.list_imports.side_effect = ClientError(
        {'Error': {'Code': 'SomeErrorCode', 'Message': 'is not '}}, 'some_operation'
    )

    with pytest.raises(ClientError):
        get_imports(export_name=export_name)
