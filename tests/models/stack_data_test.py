...
import cycl.models.stack_data as stack_data_module
from cycl.models.stack_data import StackData


@pytest.fixture
def mock_parse_name_from_id():
    with patch.object(StackData, 'parse_name_from_id') as mock:
...
def mock_boto3(cfn_client_mock):
    with patch.object(stack_data_module, 'boto3') as mock:
...
def test_parse_name_from_id(stack_id, expected):
    actual = StackData.parse_name_from_id(stack_id)
...
def test_get_all_exports_returns_an_export(list_exports_return, expected_exports, mock_boto3, cfn_client_mock):
    ...
    actual_exports = StackData.get_all_exports()
...
def test_get_all_exports_conditionally_creates_client(mock_boto3, cfn_client_mock):
    ...
    actual_exports = StackData.get_all_exports(cfn_client=cfn_client_mock)
...
@pytest.mark.parametrize(
    ...
    [
        (['some-import-stack-name'], [StackData(stack_name='some-import-stack-name')]),
        ([], []),
    ],
)
def test_get_all_imports_returns_imports(list_imports_return, expected_imports, mock_boto3, cfn_client_mock):
    ...
    actual_stack_data = StackData(stack_name='some-stack-name', export_name=export_name).get_all_imports()

    mock_boto3.client.assert_called_once_with('cloudformation')
    cfn_client_mock.list_imports.assert_called_once_with(ExportName=export_name)
    assert actual_stack_data.importing_stacks == expected_imports
...
def test_get_all_imports_uses_next_token(mock_boto3, cfn_client_mock):
    ...
    expected_imports = [StackData(stack_name=import1), StackData(stack_name=import2)]
...
def test_get_all_imports_excepts_client_error(mock_boto3, cfn_client_mock):
    ...
    mock_boto3.client.assert_called_once_with('cloudformation')
    assert actual_stack_data.importing_stacks == []
...
def test_get_all_imports_raises_client_error(mock_boto3, cfn_client_mock):
    ...
    with pytest.raises(ClientError):
        StackData(stack_name='some-stack-name', export_name=export_name).get_all_imports()
...
def test_from_list_exports(list_exports_resp, expected):
    actual = StackData.from_list_exports(list_exports_resp=list_exports_resp)
    assert actual == expected
